...
- **バグ修正**:
    ...
    - **OCRログ出力時のクラッシュ要因**: `DataManager` に存在しない属性 `stat_translation_map` を参照していたため、OCR結果の解析時にエラーになる問題を修正。適切な翻訳メソッド `self.tr()` を使用するように変更（`app_logic.py`）。
    - **言語設定による計算結果の不一致**: 英語モード時にUIからデータを抽出する際、内部キーではなく翻訳後の表示テキストを取得していたため、計算ロジック側でステータスを認識できなくなる問題を修正。`currentData()` および `findData()` を使用するように変更し、データの一貫性を確保（`tab_manager.py`）。
    - **計算結果表示の多言語化不足**: 英語モード時でも計算結果欄のステータス名やラベルが日本語のまま残っていた問題を修正。`html_renderer.py` 内のすべての表示項目に `self.tr()` を適用し、完全にローカライズされるように改善。
- **デッドコードの除去**:
- **リファクタリングとテスト**:
...
- **デッドコードの除去**:
    - **インポートの最適化**: `wuwacalc17.py`, `ui_components.py`, `app_logic.py` 等から未使用のインポートを削除し、依存関係を整理。
    - **レガシーロジックの削除**: `constants.py` から `CharacterManager` に移行済みの古い名前変換関数や重複する定数定義、大規模なコメントアウトブロックを削除。
- **疎結合化 (Decoupling)**:
...
- **多言語OCR対応の強化**:
    - **OCR言語の統合**: アプリの言語設定に関わらず、OCRエンジンに `jpn+eng` を指定するように変更。英語UI使用時でも日本語のスクリーンショットを読み取り可能に改善（`app_logic.py`）。
    - **英語名称エイリアスの追加**: `STAT_ALIASES` に英語のステータス名を追加し、日本語・英語両方のゲーム画面からステータスを正しく認識できるように拡張（`constants.py`）。
- **高度な履歴管理**:
