...
2. 「自動計算」が有効かつ切り取りモードが「％指定」の場合、現在のパーセント設定で自動的にクロップを適用。
    - 「ドラッグ」モードの場合は、自動クロップは行わずプレビューを表示し、ユーザーの切り取り実行を待機する。
3. `WorkerThread` (`OCRWorker`) がバックグラウンドで OCR を実行し、UIフリーズを防止。
4. OCR 結果が `SubStat` オブジェクトとして返され、`TabManager` が現在のタブに値をセット。
5. メインステータスはコスト設定と OCR テキストから自動推論して適用される。
6. 「自動計算」が有効な場合：
    - キャラクターが選択済みであれば、データ反映後に自動的に計算を実行。
    - キャラクターが未選択であれば、その後のキャラクター選択時に自動的に計算を実行。
...
- **UI/UX の強化**: 
    - 履歴検索・フィルタリング機能。
    - 画像読み込み時の自動クロップ・自動計算機能（Auto Calculate）。
