...
- [x] **言語切り替え時にOCR枠内（画像読込ボタン、切り取りモード等）のテキストが英語に更新されない問題を修正。**

    - `UIComponents` のUI要素をインスタンス属性化し、`retranslate_ui` ですべて更新されるように改善。

    - クロップ設定ラベル（L, T, W, H）の翻訳対応を追加。

- [x] **テキストの影（アウトライン）の品質向上と、表示設定からのオン/オフ、詳細調整機能を追加。**

    - `HtmlRenderer` にて、背景画像に強い半透明の保護レイヤー（陰）を実装し視認性を確保。

    - `AppConfig` に詳細パラメータ（オフセット、ぼかし、拡張）を追加し、アプリ全体（グローバル）に `QGraphicsDropShadowEffect` を適用。

- [x] **テーマ管理とカスタマイズ体験の向上。**

    - 個別設定変更時に自動で「カスタム」テーマへ切り替わるロジックを実装。

    - プリセット選択時に文字色や影の色を自動最適化する連動機能を追加。

- [x] **フレーム透過機能の刷新。**

    - `QGroupBox` や `QTextEdit` の背景を透過させ、背景画像を最大限活かすデザインへ。

    - 操作性維持のため、タブエリアの構造線のみを控えめに残す調整を実施。
