...
- **バグ修正**: `core/app_logic.py` で存在しない `LOG_LEVEL_INFO`, `LOG_LEVEL_ERROR` をインポートし `ImportError` が発生する問題を修正。
- **UX改善 (OCR & キャラクタ選択)**: 起動時にキャラクター未指定の場合は `General` がデフォルト設定されるようになり、事前のキャラ指定なしに画像読み込み・OCRが可能になった。さらに、文字や画像を読み込んだ後にキャラクターをコンボボックスで変更すると、再OCRなしで即時に該当キャラクター用のスコアが再計算される「遅延評価（Deferred Calculation）」ワークフローを導入。
- **UI表示拡張 (Batchモード)**: Batch評価時のHTML出力において、有効になっている詳細な評価指標（Normalized, Ratio, Roll, Effective, CV）が省略されてしまう問題を修正し、個別の音骸ブロックおよび最終の平均値サマリーの両方に全指標を出力するように拡張。
